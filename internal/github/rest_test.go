...
func TestRESTClientRejectsPlaintextGitHubPublicAPIBeforeRequest(t *testing.T) {
	for _, baseURL := range []string{"http://api.github.com", "http://API.GITHUB.COM", "http://api.github.com."} {
		t.Run(baseURL, func(t *testing.T) {
			...
			client := NewRESTClient(baseURL, "plain-text-secret", "2022-11-28", &http.Client{Transport: transport})
...
func TestValidateSafeDraftPRRequestUsesSharedCanonicalBounds(t *testing.T) {
	valid := DraftPRRequest{Title: "Revert merge", Body: "Reason:\n<pre>safe</pre>", Head: "revert/184", Base: "main", IssueNumber: 184}
	if err := ValidateSafeDraftPRRequest(repo(), valid); err != nil {
		t.Fatal(err)
	}
	for name, request := range map[string]DraftPRRequest{
		"title whitespace": {Title: " Revert", Body: valid.Body, Head: valid.Head, Base: valid.Base, IssueNumber: 184},
		"body whitespace":  {Title: valid.Title, Body: " " + valid.Body, Head: valid.Head, Base: valid.Base, IssueNumber: 184},
		"body expansion":   {Title: valid.Title, Body: strings.Repeat("<", MaxDraftPRBodyBytes+1), Head: valid.Head, Base: valid.Base, IssueNumber: 184},
		"missing number":   {Title: valid.Title, Body: valid.Body, Head: valid.Head, Base: valid.Base},
	} {
		t.Run(name, func(t *testing.T) {
			if err := ValidateSafeDraftPRRequest(repo(), request); err == nil {
				t.Fatal("accepted invalid draft request")
			}
		})
	}
}

func TestCreateSafeDraftPRRequiresExactSuccessIdentity(t *testing.T) {
	request := DraftPRRequest{Title: "Revert merge", Body: "body", Head: "revert/184", Base: "main", IssueNumber: 184}
	for name, response := range map[string]string{
		"zero number": `{"number":0,"draft":true,"head":{"ref":"revert/184"},"base":{"ref":"main"}}`,
		"non draft":   `{"number":1,"draft":false,"head":{"ref":"revert/184"},"base":{"ref":"main"}}`,
		"wrong head":  `{"number":1,"draft":true,"head":{"ref":"other"},"base":{"ref":"main"}}`,
		"wrong base":  `{"number":1,"draft":true,"head":{"ref":"revert/184"},"base":{"ref":"develop"}}`,
		"valid":       `{"number":1,"draft":true,"head":{"ref":"revert/184"},"base":{"ref":"main"}}`,
	} {
		t.Run(name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) { _, _ = io.WriteString(w, response) }))
			defer server.Close()
			client := NewRESTClient(server.URL, "token", "2022-11-28", server.Client())
			got, err := client.CreateSafeDraftPR(context.Background(), repo(), request)
			if name == "valid" {
				if err != nil || got.Number != 1 {
					t.Fatalf("pr=%+v err=%v", got, err)
				}
			} else if err == nil {
				t.Fatalf("accepted unsafe identity=%+v", got)
			}
		})
	}
}
