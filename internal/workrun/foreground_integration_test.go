...
type scriptedForegroundRuntime struct{ launches, observes int }
...
func (r *scriptedForegroundRuntime) Observe(context.Context, statev2.InvocationState, runtimecontract.Invocation) (coordinator.RuntimeObservation, error) {
	r.observes++
...
func TestForegroundRunRealStoreGitAndCoordinatorReplaysWithoutLaunch(t *testing.T) {
	...
	if rt.launches != 1 || rt.observes != 1 {
		t.Fatalf("reconcile launch/observe=%d/%d", rt.launches, rt.observes)
	}
}
