...
func printUsage(stderr io.Writer) {
	fmt.Fprintln(stderr, "사용법: agentctl version | contract <validate|preview> <file> | project register PROJECT.json --expected-revision 0 --request-id ID | project list --json | project status --all --json | work plan CONTRACT.json --expected-revision 0 --request-id ID | work approve WORK --expected-revision N --request-id ID | work run WORK --expected-revision N --request-id ID | work pause WORK --expected-revision N --request-id ID | work resume WORK --expected-revision N --request-id ID | work reconcile WORK --json | work status WORK --json | start CONTRACT | status [RUN] [--json] | stop RUN | resume RUN | retire RUN [--blocked] | cleanup RUN | confirm RUN protected-change | create-revert RUN --reason TEXT")
}
