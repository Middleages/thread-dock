...
func printUsage(stderr io.Writer) {
	fmt.Fprintln(stderr, "사용법: agentctl version | contract <validate|preview> <file> | project register PROJECT.json | project list --json | project status --all --json | work plan CONTRACT.json | work approve WORK --expected-revision N --request-id ID | work status WORK --json | start CONTRACT | status [RUN] [--json] | stop RUN | resume RUN | retire RUN [--blocked] | cleanup RUN | confirm RUN protected-change | create-revert RUN --reason TEXT")
}
