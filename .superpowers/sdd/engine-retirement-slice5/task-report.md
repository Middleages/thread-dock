...
- Added no replacement implementation, moved logic, or new tests; this is a pure deletion.
...
Go 1.27.0 toolchain. The following is the shell setup and command form used during the
verification (the generated tmp_dir was the same independent /dev/shm directory for
each command):

    go_bin=/home/appuser/.local/share/threaddock/toolchains/go1.27.0/bin/go
    tmp_dir=$(mktemp -d /dev/shm/engine-retirement-v1-integration.XXXXXX)
    trap 'rmdir "$tmp_dir"' EXIT

    TMPDIR="$tmp_dir" "$go_bin" test ./internal/workrun -run '^TestReviewIntegration' -v
    PASS: 13 TestReviewIntegration* cases ran and passed

    TMPDIR="$tmp_dir" "$go_bin" vet ./internal/workrun
    PASS: exit 0

    TMPDIR="$tmp_dir" "$go_bin" list ./...
...
- NewReviewIntegrationService, ReviewIntegrationService.Advance, the current workrun
  integration flow, and MergeCommitNoFF/AbortMerge/CurrentCommit remain.
