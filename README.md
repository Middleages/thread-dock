...
- 프로젝트 상세의 `보기`는 0개 또는 완료 항목만 있어도 같은 전역 Drawer의 해당 프로젝트 필터로 연결됩니다. 개수 미조회는 실제 0과 구분하며, GitHub 업무 상태와도 별개입니다.
...
이전 `projects.json` v1의 명령어·체크리스트는 처음 새 Toolbox 경로에 접근할 때 전역 파일로 옮깁니다. 전역 저장이 성공한 뒤 프로젝트 파일을 자료 전용 v2로 바꾸며, 중간 실패 시 재시도해 중복을 만들지 않습니다. 정상 legacy 항목은 합산 개수 때문에 버리거나 migration을 거부하지 않습니다. 새 전역 저장은 종류별 10,000개 기준을 사용하며, 이미 초과한 데이터도 읽기·편집·삭제할 수 있고 추가 증가만 제한합니다. 잘못된 파일·지원하지 않는 버전·유효하지 않은 항목은 오류로 남기고 원본을 조용히 덮거나 잘라내지 않습니다. SQLite는 사용하지 않습니다.
