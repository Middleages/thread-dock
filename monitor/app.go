...
type App struct {
	source monitor.SnapshotSource
}

func NewApp(source monitor.SnapshotSource) *App {
	return &App{source: source}
}

// GetMonitorSnapshot returns the provider-neutral aggregate wire for the UI.
func (a *App) GetMonitorSnapshot() (monitor.Snapshot, error) {
	if a == nil || a.source == nil {
		return monitor.Snapshot{}, errors.New("monitor snapshot source is nil")
	}
