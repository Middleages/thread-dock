...
const severity: Record<string, number> = {
  conflict: 5, blocked: 5, failed: 5, offline: 4, degraded: 4, setup_required: 4, disabled: 4,
  stale: 3, cached: 3, unverified: 2, missing: 2, unknown: 1,
  fresh: 0, synced: 0, connected: 0, working: 0, idle: 0, done: 0, running: 0,
}
const stateRank = (value?: string) => value
  ? Object.prototype.hasOwnProperty.call(severity, value) ? severity[value] : 1
  : 0
...
function connectionState(connection: HerdrConnection) {
  // `agentStatus` is optional. An absent agent observation must not turn a
  // proven connection into unknown, while an explicit future value remains a
  // warning until the producer's vocabulary is understood.
  const states = [connection.status || 'unknown']
  if (connection.agentStatus) states.push(connection.agentStatus)
