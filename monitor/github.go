...
func parseMonitorConfig(env map[string]string) (monitorConfig, string) {
	...
	var unknown []string
	for key := range env {
		if strings.HasPrefix(key, "THREADDOCK_") && !known[key] { unknown = append(unknown, key) }
	}
...
func parseRepos(raw string) ([]repositoryTarget, error) {
	...
	for _, item := range splitConfig(raw) {
		...
		if match == nil { return nil, fmt.Errorf("THREADDOCK_REPOS must contain OWNER/REPO entries: %s", item) }
...
func parseProjects(raw string) ([]projectTarget, error) {
	...
	for _, item := range splitConfig(raw) {
		...
		if match == nil { return nil, errors.New("THREADDOCK_PROJECTS must contain GitHub user or org project URLs") }
...
func (m *GitHubMonitor) completeAggregate(call *aggregateCall) {
	...
	m.mu.Lock()
	if m.inFlight == call { m.inFlight = nil }
	m.mu.Unlock()
	close(call.done)
}
...
func (m *GitHubMonitor) load() (Snapshot, error) {
	...
	for _, repo := range m.config.repos {
		...
		if project.State == "stale" { stale = true }
	...
	m.mu.Unlock()
	observed := now
	for i := range projects { notices = append(notices, projects[i].Notices...) }
...
func (m *GitHubMonitor) setupSnapshot(now time.Time, reason string) Snapshot {
	m.mu.Lock(); m.revision++; revision := m.revision; m.mu.Unlock()
...
func (m *GitHubMonitor) fetchRepository(repo repositoryTarget, notices *[]string) Project {
	...
	for _, pr := range prRows {
		...
		for _, ref := range pr.ClosingIssuesReferences {
			if issueURL := safeGitHubURL(ref.URL); issueURL != "" && prURL != "" { prsByIssue[issueURL] = append(prsByIssue[issueURL], prURL) }
		}
	}
	items := make([]WorkItem, 0, len(issueRows)+len(prRows))
	for _, issue := range issueRows { items = append(items, mapIssue(key, issue, issues, prsByIssue[safeGitHubURL(issue.URL)])) }
	for _, pr := range prRows { items = append(items, mapPullRequest(key, pr, prs)) }
	updated := laterTime(issues.observedAtPtr(), prs.observedAtPtr())
	isStale := issues.stale || prs.stale
	if len(issueRows) >= 100 { *notices = append(*notices, key+" 이슈 조회 결과가 최대 100개로 제한됐습니다.") }
	if len(prRows) >= 100 { *notices = append(*notices, key+" PR 조회 결과가 최대 100개로 제한됐습니다.") }
	projectNotices := []string{}
	if isStale { projectNotices = append(projectNotices, key+"의 일부 GitHub 조회가 오래된 상태입니다.") }
...
func (m *GitHubMonitor) fetchProject(board projectTarget, notices *[]string) Project {
	...
	if payload.totalCount > 100 || len(payload.items) >= 100 { *notices = append(*notices, key+" 조회 결과가 최대 100개로 제한됐습니다.") }
	inaccessible := 0
	items := make([]WorkItem, 0, len(payload.items))
	for index, item := range payload.items {
		if itemMap, ok := item.(map[string]any); !ok || itemMap["content"] == nil { inaccessible++ }
		items = append(items, mapProjectItem(item, board, index, result))
	}
	if inaccessible > 0 { *notices = append(*notices, fmt.Sprintf("%s에서 %d개 항목의 내용을 확인할 수 없습니다.", key, inaccessible)) }
	projectNotices := []string{}
	if result.stale { projectNotices = append(projectNotices, key+" 조회가 오래된 상태입니다.") }
...
func (m *GitHubMonitor) readSource(key string, args []string, decode func([]byte) (any, error), notices *[]string) sourceRead {
	now := m.now()
	m.mu.Lock(); cached, exists := m.cache[key]; m.mu.Unlock()
	if exists && now.Before(cached.nextAttemptAt) { return sourceRead{data: cached.data, observedAt: cached.observedAt, stale: cached.stale, hasData: cached.hasData} }
	if exists && cached.hasData && now.Sub(cached.observedAt) < githubTTL { return sourceRead{data: cached.data, observedAt: cached.observedAt, hasData: true} }
	output, err := m.command.run(context.Background(), args[0], args[1:]...)
	var data any
	if err == nil { data, err = decode(output) }
	if err == nil {
		entry := cachedSource{data: data, observedAt: now, nextAttemptAt: now.Add(githubTTL), hasData: true}
		m.mu.Lock(); m.cache[key] = entry; m.mu.Unlock()
	...
	*notices = append(*notices, key+" 조회에 실패했습니다. 마지막 성공 시각이 있으면 보존된 데이터를 표시합니다.")
	cached.nextAttemptAt = now.Add(githubTTL); cached.stale = true
	m.mu.Lock(); m.cache[key] = cached; m.mu.Unlock()
	return sourceRead{data: cached.data, observedAt: cached.observedAt, stale: true, hasData: cached.hasData}
}

type rawIssue struct { Number *int `json:"number"`; Title, URL, State, Body, UpdatedAt string; Author rawAuthor `json:"author"` }
type rawAuthor struct { Login string `json:"login"` }
...
func mapPullRequest(repo string, pr rawPullRequest, source sourceRead) WorkItem {
	url := safeGitHubURL(pr.URL); related := []string{}
	for _, ref := range pr.ClosingIssuesReferences { if value := safeGitHubURL(ref.URL); value != "" { related = append(related, value) } }
...
func mapProjectItem(value any, board projectTarget, index int, source sourceRead) WorkItem {
	item, _ := value.(map[string]any); content, _ := item["content"].(map[string]any)
	url := safeGitHubURL(textValue(content["url"])); number := intPointer(content["number"])
	identity := urlOrFallback(url, textValue(item["id"])); if identity == "" { identity = strconv.Itoa(index) }
	title := textValue(content["title"]); if title == "" { title = textValue(content["name"]) }; if title == "" { title = "GitHub project item (content unavailable)" }
...
func baseWork(kind, identity, title, body, updated string, github *GitHubWork, source sourceRead) WorkItem {
	if title == "" { title = identity }
	links := []Link{}
	if github.URL != "" { links = append(links, Link{Kind: kind, Label: map[string]string{"pr": "Pull request", "issue": "Issue", "project-item": "GitHub"}[kind], URL: github.URL}) }
	for _, bodyURL := range bodyURLs(body) { if bodyURL != github.URL { links = append(links, Link{Kind: "evidence", Label: "본문 링크", URL: bodyURL}) } }
...
func projectFields(item map[string]any) map[string]string {
	...
	if values, ok := item["fieldValues"].([]any); ok { for _, raw := range values { if field, ok := raw.(map[string]any); ok { name := textValue(fieldPath(field, "field", "name")); if name == "" { name = textValue(field["name"]) }; if name != "" { fields[name] = projectFieldValue(field) } } } }
	...
	for name, value := range item { if !structural[name] { if rendered := projectFieldValue(value); rendered != "" { fields[name] = rendered } } }
	return fields
}
...
func projectFieldValue(value any) string {
	...
	if object, ok := value.(map[string]any); ok {
		for _, key := range []string{"name", "value", "text", "number", "date"} { if text := textValue(object[key]); text != "" { return text } }
		if users, ok := object["users"].([]any); ok { return joinObjectValues(users, "login", "name") }
		if labels, ok := object["labels"].([]any); ok { return joinObjectValues(labels, "name") }
		return ""
	}
	if values, ok := value.([]any); ok { parts := make([]string, 0, len(values)); for _, item := range values { if text := projectFieldValue(item); text != "" { parts = append(parts, text) } }; return strings.Join(parts, ", ") }
...
func safeGitHubURL(value string) string { parsed, err := url.Parse(value); if err != nil || parsed.Scheme != "https" || parsed.Host != "github.com" || !strings.HasPrefix(parsed.Path, "/") { return "" }; return value }
func stateFor(value string) string { value = strings.ToLower(strings.TrimSpace(value)); if value == "" { return "unknown" }; return value }
func textValue(value any) string { switch value := value.(type) { case string: return value; case float64: return strconv.FormatFloat(value, 'f', -1, 64); case json.Number: return value.String(); case bool: return strconv.FormatBool(value); default: return "" } }
func firstText(object map[string]any, keys ...string) string { for _, key := range keys { if value := textValue(object[key]); value != "" { return value } }; return "" }
func numberValue(value any) int { text := textValue(value); result, _ := strconv.Atoi(text); return result }
func intPointer(value any) *int { result := numberValue(value); if result == 0 { return nil }; return &result }
func numberText(value *int) string { if value == nil { return "" }; return strconv.Itoa(*value) }
func urlOrFallback(value, fallback string) string { if value != "" { return value }; return fallback }
func nonNilStrings(value string) []string { if value == "" { return []string{} }; return []string{value} }
func uniqueStrings(values []string) []string { result := []string{}; seen := map[string]bool{}; for _, value := range values { if value != "" && !seen[value] { seen[value] = true; result = append(result, value) } }; return result }
func parseTime(value string) *time.Time { if value == "" { return nil }; parsed, err := time.Parse(time.RFC3339, value); if err != nil { return nil }; return &parsed }
func timePtr(value time.Time) *time.Time { return &value }
func (r sourceRead) observedAtPtr() *time.Time { if r.observedAt.IsZero() { return nil }; return timePtr(r.observedAt) }
func laterTime(a, b *time.Time) *time.Time { if a == nil { return b }; if b == nil || !b.After(*a) { return a }; return b }
func projectEvidence(projects []Project) []string { result := []string{}; for _, project := range projects { result = append(result, project.EvidenceRefs...) }; return result }
func choose(condition bool, yes, no string) string { if condition { return yes }; return no }
