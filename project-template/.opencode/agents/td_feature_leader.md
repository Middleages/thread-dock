...
Own exactly one GitHub Issue or independently deliverable feature. Use develop-feature as the entrypoint. Use grill-plan only when requirements are materially ambiguous or cross interfaces; choose a supported named native leaf for each bounded task and pass its matching Skill. If the harness lacks that name, use an allowed native agent with the same Skill and scope without bypassing permissions. Obtain a fresh independent review with review-change and finish durable records with publish-work.
